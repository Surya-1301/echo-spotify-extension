...
class CipherPair(sendKey: ByteArray, receiveKey: ByteArray) {
    ...
    suspend fun receiveEncoded(`in`: DataInputStream) = receiveMutex.withLock {
        withContext(Dispatchers.IO) {
            ...
            `in`.readFully(header)
            receiveCipher.decrypt(header)

            val cmd = header[0]
            val payloadSize = ((header[1].toInt() shl 8) or (header[2].toInt() and 0xff))
            val payload = ByteArray(payloadSize)
            `in`.readFully(payload)
            receiveCipher.decrypt(payload)
            val mac = ByteArray(4)
            `in`.readFully(mac)
            val expectedMac = ByteArray(4)
            receiveCipher.finish(expectedMac)
            if (!mac.contentEquals(expectedMac)) {
                throw GeneralSecurityException("MACs don't match!")
            }
